...
* **开发语言**：ArkTS
* **目标 SDK**：HarmonyOS 5.0.1 Release
...
1.  **home (首页)**：提供扫一扫、身份识别、社保查询、居住证办理、二维码生成等常用功能入口。
2.  **office (办事)**：提供出行、养老、房产、就业等政务办事功能的页面框架。
3.  **mine (个人中心)**：包含用户登录、注册、账户安全、隐私设置、个人信息管理等用户侧功能。
4.  **message (消息)**：用于展示系统通知或消息列表。
5.  **RouterModule (路由)**：封装了统一的页面路由跳转逻辑，负责模块间的导航协调。
